...
class TwoDimensionalEnvironment(object):
    ...
    def simulate(self, agent: TwoDimensionalAgent) -> Tuple[int, float]:
        ...
        for i, sample in enumerate(self.food_loadout):
            ...
            for time_step in range(i * NUM_TIME_STEPS, (i + 1) * NUM_TIME_STEPS):
                ...
                agent.eat_actuator = TwoDimensionalEnvironment._count_spikes_within_time_window(time_step, eat_actuator)
                agent.avoid_actuator = TwoDimensionalEnvironment._count_spikes_within_time_window(time_step, avoid_actuator)
                self.deal_damage(agent, sample)
            str_correct_wrong = "CORRECT" if (
                                    agent.select_action() is Action.EAT and sample is self.current_logic_gate) or (
                                    agent.select_action() is Action.AVOID and sample is not self.current_logic_gate) \
                                else "WRONG"
            print(f'Agent health: {int(agent.health_points)}, i={i}, beneficial food: {self.current_logic_gate}, sample: {sample}, action: {agent.select_action()} {str_correct_wrong}')
    ...
    def simulate_with_visualization(self, agent: OneDimensionalAgent) -> Tuple[int, float, dict, dict, int]:
        ...
        for i, sample in enumerate(self.food_loadout):
            ...
            print(f'Agent health: {int(agent.health_points)}, i={i}, current_logic_gate: {self.current_logic_gate}, sample: {sample}, action: {agent.select_action()} {str_correct_wrong}')
            print(f'Eat: {agent.eat_actuator}, Avoid: {agent.avoid_actuator}')
    ...
    def _get_minimum_lifetime(self):
        beneficial_food = self.current_logic_gate
